...
//
// ✅ 本站**有** public/,且 scripts/bundle-static.mjs 會 copyDir(public → site 根)
//   所以輸出到 public/tts/ 就會自動進 site/tts/(已於 build 後實際驗過)。
...
// 🧹 開場先掃掉上一輪殘留的 _tmp_* 暫存夾。
// ★ 由來(0813 實測):這台機器烤一句 node 就會被 libuv 打斷(msedge-tts 的 WebSocket),
//   `finally` 的清理**來不及跑** ⇒ 每次崩潰都留下一個空的 _tmp_<key>/。
//   六句烤完就留六個,而且 bundle-static 的 copyDir 是**整包複製 public/**
//   ⇒ 這些垃圾會一路跟著上線。清理必須放在「開場」而不是只放 finally,
//   因為會殺死 finally 的正是那個崩潰本身。
