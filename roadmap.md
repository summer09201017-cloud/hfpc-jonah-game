...
- **主動衝刺(按住不放)**(2026-06-11):闖關模式手指按住螢幕(或按住 →)超過 0.22 秒=持續 ×1.3 加速,放開恢復;輕點仍是跳(`config.SPRINT`)。與 ⚡ 同時取較大倍率不疊乘。(設計筆記:不用「連點加速」——連點會跟「點=跳」衝突,狂點會瘋狂跳躍。)
...
- **視覺(2026-06-11)**:烏雲密布(兩層漂移烏雲)、暴風大雨(130 道斜雨,更粗更快)、**船放大 1.5 倍**、鼓脹的帆與船板紋、船尾欄杆。
- **驚惶的水手改向量小人**(`renderer._sailor`,不再用 emoji):跪下雙手朝天哀求(揮動)、俯伏甲板禱告(前後搖)、雙手死抓欄杆身體被浪甩、**把貨物拋進海裡(循環飛出的貨箱,拿 1:5)**、**古代船主**(深紅長袍+白頭巾+腰帶,朝約拿焦急揮手喊 ❗,拿 1:6)——動作全部隨時間慌張擺動,有緊張感。
